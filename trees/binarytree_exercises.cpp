...
Node* lowest_common_ancestor(Node *p, Node *q) {
    // Lowest common ancestor => first node that contains both of two marked nodes
    if (p==NULL || q==NULL){
        return NULL;
    }
    if (p == q){
...
Node* create_bst_helper(int data[], int low, int high, int level) {
    ...
    Node *parent = new Node(level, data[m]);
    parent->left = create_bst_helper(data, low, m-1, level+1);
    parent->right = create_bst_helper(data, m+1, high, level+1);
    return parent;
}
...
int main(int argc, const char *argv[]) {
    ...
    int data[15] = {11, 18, 19, 20, 24, 25, 67, 68, 69, 93};
    Node *bst = create_bst(data, 10);
